...
create table if not exists public.semantic_assertion_evidence (
  ...
  transcript_chunk_id uuid references public.asset_transcript_chunks(id) on delete set null,
...
create table if not exists public.semantic_embeddings (
 id uuid primary key default gen_random_uuid(), asset_id uuid not null references public.assets(id) on delete cascade,
 scene_id uuid references public.asset_scenes(id) on delete cascade, event_id uuid references public.asset_events(id) on delete cascade,
 transcript_chunk_id uuid references public.asset_transcript_chunks(id) on delete cascade, ocr_observation_id uuid references public.ocr_observations(id) on delete cascade,
...
select distinct on (a.asset_id,a.layer_id,a.predicate,coalesce(a.subject_id,a.asset_id))
 ...
 coalesce(g.canonical_concept_code,a.canonical_concept_code) canonical_concept_code,
 case when g.id is not null then g.value_json else coalesce(a.value_json,to_jsonb(a.value_text),to_jsonb(a.value_number),to_jsonb(a.value_boolean)) end effective_value,
 coalesce(g.semantic_state,a.semantic_state) semantic_state,a.confidence,a.search_critical,a.analysis_run_id,
 case when g.id is not null then 'HUMAN_GOLD' when a.human_review_status in ('APPROVED','CORRECTED') then 'APPROVED_AI' else 'UNVERIFIED_AI' end resolution_source
...
order by a.asset_id,a.layer_id,a.predicate,coalesce(a.subject_id,a.asset_id),
 case when g.id is not null then 1 when a.human_review_status in ('APPROVED','CORRECTED') then 2 else 3 end,a.created_at desc;
...
create policy semantic_assertions_asset_read on public.semantic_assertions for select to authenticated using(public.can_access_asset(asset_id));
create policy semantic_evidence_asset_read on public.semantic_assertion_evidence for select to authenticated using(public.can_access_asset(asset_id));
create policy semantic_layers_asset_read on public.asset_semantic_layers for select to authenticated using(public.can_access_asset(asset_id));
create policy semantic_events_asset_read on public.asset_events for select to authenticated using(public.can_access_asset(asset_id));
create policy semantic_narratives_asset_read on public.semantic_narratives for select to authenticated using(public.can_access_asset(asset_id));
create policy search_concepts_v2_asset_read on public.asset_search_concepts_v2 for select to authenticated using(public.can_access_asset(asset_id));
