...
begin
  ...
  if tg_table_name='asset_keyframes' and (new.timestamp_seconds < v_start or new.timestamp_seconds > v_end) then raise exception using errcode='23514', message='keyframe timestamp must fall within scene'; end if;
  if tg_table_name='scene_actions' and ((new.start_seconds is not null and new.start_seconds < v_start) or (new.end_seconds is not null and new.end_seconds > v_end)) then raise exception using errcode='23514', message='action timestamps must fall within scene'; end if;
  if tg_table_name='asset_transcript_chunks' and ((new.start_seconds is not null and new.start_seconds < v_start) or (new.end_seconds is not null and new.end_seconds > v_end)) then raise exception using errcode='23514', message='transcript timestamps must fall within scene'; end if;
  if tg_table_name='ocr_observations' and new.timestamp_seconds is not null and (new.timestamp_seconds < v_start or new.timestamp_seconds > v_end) then raise exception using errcode='23514', message='OCR timestamp must fall within scene'; end if;
  return new;
end $$;
