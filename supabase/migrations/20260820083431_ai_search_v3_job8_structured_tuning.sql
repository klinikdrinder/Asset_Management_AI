...
select id,round((final*100)::numeric,2),round((av*100)::numeric,2),round((sv*100)::numeric,2),round((kv*100)::numeric,2),
 ...
 features||jsonb_build_object('ranking_version','job8-v3.4','signals',jsonb_build_object('visual_asset',av,'visual_scene',sv,'visual_keyframe',kv,'lexical_asset',al,'lexical_scene',sl,'structured',ss,'filename',fn)),
...
'KDI AI Search V3 job8-v3.4: permission-prefiltered OpenCLIP visual, PostgreSQL lexical, ontology/structured hybrid ranking with image-level structured evidence and domain-aware visual-only thresholds. Weights unchanged from job7-v3.3. Default 5, max 50.';
