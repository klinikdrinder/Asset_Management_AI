...
select id,round((final*100)::numeric,2),round((av*100)::numeric,2),round((sv*100)::numeric,2),round((kv*100)::numeric,2),
 ...
 features||jsonb_build_object('ranking_version','job7-v3.3','signals',jsonb_build_object('visual_asset',av,'visual_scene',sv,'visual_keyframe',kv,'lexical_asset',al,'lexical_scene',sl,'structured',ss,'filename',fn)),
...
'KDI AI Search V3 job7-v3.3: permission-prefiltered OpenCLIP visual, PostgreSQL lexical, ontology/structured hybrid ranking with a domain-aware visual-only threshold. Default 5, max 50.';
