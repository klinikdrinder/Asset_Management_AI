-- KDI AI Search V3 Job 7: permission-first hybrid retrieval.
...
with params as (
  ...
  cross join lateral (select
    not(p.f?'treatment') or exists(select 1 from public.scene_treatments st join public.treatments t on t.id=st.treatment_id where st.asset_id=a.id and lower(t.code)=lower(p.f->>'treatment')) treatment_match,
    not(p.f?'anatomy') or exists(select 1 from public.scene_anatomy sa join public.anatomy_terms an on an.id=sa.anatomy_id left join public.anatomy_terms parent on parent.id=an.parent_id where sa.asset_id=a.id and (lower(an.code)=lower(p.f->>'anatomy') or lower(parent.code)=lower(p.f->>'anatomy'))) anatomy_match,
    not(p.f?'action') or exists(select 1 from public.scene_actions x join public.actions ac on ac.id=x.action_id where x.asset_id=a.id and lower(ac.code)=lower(p.f->>'action')) action_match,
    ...
    not(p.f?'environment') or exists(select 1 from public.scene_environment e where e.asset_id=a.id and lower(e.environment_type) like '%'||lower(p.f->>'environment')||'%') environment_match,
 ...
 where final>=greatest(p.threshold,case when ss=0 and greatest(al,sl,fn)=0 then 0.18 else 0 end)
   and (p.q is not null or visual_query_embedding is not null or ss>0)
   and (not (p.f ?| array['treatment','anatomy','action','role','scene_type','environment','shot_type','content_type']) or ss=1)
), ranked as (
...
select id,round((final*100)::numeric,2),round((av*100)::numeric,2),round((sv*100)::numeric,2),round((kv*100)::numeric,2),
 ...
 features||jsonb_build_object('ranking_version','job7-v3.2','signals',jsonb_build_object('visual_asset',av,'visual_scene',sv,'visual_keyframe',kv,'lexical_asset',al,'lexical_scene',sl,'structured',ss,'filename',fn)),
...
'KDI AI Search V3 job7-v3.2: permission-prefiltered OpenCLIP visual, PostgreSQL lexical, ontology/structured hybrid ranking with a corroboration-aware visual-only threshold. Default 5, max 50.';
