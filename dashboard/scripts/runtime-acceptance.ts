...
try{const token=await getDriveAccessToken(env);const driveId=resolveKdiMasterDriveId(env);const response=await fetch(`https://www.googleapis.com/drive/v3/drives/${encodeURIComponent(driveId)}?fields=id`,{headers:{Authorization:`Bearer ${token}`}});if(response.ok)result.GOOGLE_DRIVE_AUTH="PASS"}catch{}
...
if(session){const headers={Cookie:cookiesFor(session)};for(const route of ["/library","/library?query=IMG","/library?category=image","/library?refine=clinic%20room"]){const response=await fetch(`http://127.0.0.1:3000${route}`,{headers,redirect:"manual"});runtime[route]=response.status;const body=await response.text();if(route==="/library")runtime.asset_cards=body.includes(assetName)||/fileCard|fileGrid|Open preview|Download/.test(body)?"PASS":"FAIL"}
  if(assetId)for(const operation of ["thumbnail","preview","download"]){const response=await fetch(`http://127.0.0.1:3000/api/media/${assetId}/${operation}`,{headers,redirect:"manual"});runtime[operation]=response.status;}
}
