...
const dashboardMigration=path.join(process.cwd(),"supabase","migrations",migration),rootMigration=path.join(root,"supabase","migrations",migration);const query=readFileSync(existsSync(dashboardMigration)?dashboardMigration:rootMigration,"utf8");
...
console.log(`database_migration=${migration} applied=PASS`);
