...
export async function recordAuthDiagnostic(entry: Diagnostic) {
  // Portable default: the running dashboard process's working directory
  // (…/dashboard/.logs). KDI_AUTH_LOG_DIR still overrides when configured.
  const directory = process.env.KDI_AUTH_LOG_DIR || path.join(process.cwd(), ".logs");
