...
export const ASSET_SELECT = "id,file_name,mime_type,file_extension,size_bytes,upload_status,created_at,updated_at,asset_sources(source_file_id,source_files(id,source_folder_id,drive_created_at,drive_modified_at,source_folders(source_name))),asset_destinations(id,destination_google_file_id,upload_status,verification_level),asset_semantic_index(short_caption)";
const select = ASSET_SELECT;
export function mapAsset(row: Json, canDownload: boolean, matchPercent: number | null = null): MediaAsset {
  ...
  const semantic = objects(row.asset_semantic_index)[0] ?? {};
