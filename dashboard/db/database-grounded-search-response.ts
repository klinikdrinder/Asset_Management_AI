...
export class SupabaseGroundedResponseRepository implements GroundedResponseRepository{
  ...
  assertions(ids:string[]){return many(this.db.from("semantic_assertions").select("id,asset_id,scene_id,event_id,layer_id,predicate,canonical_concept_code,semantic_state,search_critical,confidence").in("asset_id",ids).eq("active",true).is("superseded_by",null))}
